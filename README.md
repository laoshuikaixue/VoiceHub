...
{ pkgs, inputs, ... }: {
  ...
  services.voicehub = {
    enable = true;
    port = 3000;
    environmentFile = "/run/secrets/voicehub-env";  # 包含 DATABASE_URL、JWT_SECRET 等敏感变量
...
推荐使用 [sops-nix](https://github.com/Mic92/sops-nix) 等秘密管理工具生成环境文件，避免 secrets 明文存储在 Nix store 中：

```nix
{
  sops.secrets.voicehub-env = {
    format = "dotenv";
    sopsFile = ./secrets/voicehub.env;
  };

  services.voicehub = {
    enable = true;
    environmentFile = config.sops.secrets.voicehub-env.path;
  };
}
```
