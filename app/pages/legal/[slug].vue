<template><main class="legal-page"><header class="legal-nav"><NuxtLink to="/" class="brand"><img :src="logoUrl" alt="" /><span>{{ siteTitle }}</span></NuxtLink><NuxtLink to="/login" class="login-link btn-primary"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 17l5-5-5-5M15 12H3M21 3v18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>{{ locale.login }}</NuxtLink></header><section class="legal-hero"><div class="doc-icon"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 3h9l3 3v15H6zM9 13h6M9 17h4" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" /></svg></div><div class="eyebrow">{{ locale.category }}</div><h1>{{ document?.name || locale.title }}</h1><p v-if="updatedDate">{{ locale.updated }} {{ updatedDate }}</p></section><section class="legal-body"><AppSpinner v-if="loading" /><article v-else-if="document" class="legal-content"><div class="markdown-body" v-html="renderedContent" /></article><p v-else class="empty">{{ locale.notFound }}</p></section></main></template>
<script setup>
