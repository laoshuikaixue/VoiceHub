...
export const usePermissions = () => {
  ...
  const canAccessPage = (page: string): boolean => {
    ...
    // 管理员可以访问的页面
    if (role === 'ADMIN') {
      return ['overview', 'schedule', 'print', 'songs', 'users', 'playtimes', 'request-times', 'semesters', 'data-analysis', 'song-quotas', 'card-codes', 'music-source'].includes(page)
    }

    // 歌曲管理员可以访问的页面
    if (role === 'SONG_ADMIN') {
      return ['overview', 'schedule', 'print', 'songs', 'playtimes', 'request-times', 'semesters', 'data-analysis', 'song-quotas', 'card-codes'].includes(page)
    }
  ...
  const getUserPages = computed(() => {
    ...
    switch (role) {
      ...
      case 'ADMIN':
        return ['overview', 'schedule', 'print', 'songs', 'users', 'playtimes', 'request-times', 'semesters', 'data-analysis', 'song-quotas', 'card-codes', 'music-source']
      case 'SONG_ADMIN':
        return ['overview', 'schedule', 'print', 'songs', 'playtimes', 'request-times', 'semesters', 'data-analysis', 'song-quotas', 'card-codes']
      default:
        return []
    }
  })
