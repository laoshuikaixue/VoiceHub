export const siteConfig = {
  ...
  esaCaptchaDesc: 'When enabled, all users must pass the ESA AI CAPTCHA on every login and registration. One ESA rule covers exactly one endpoint plus one domain, so add a rule in the ESA console for the login endpoint, the registration endpoint, and each public domain, then record their Scene IDs in the table below. The CAPTCHA type (traceless / click-and-pass / slider / puzzle) is decided by the rule the Scene ID belongs to and is delivered by ESA when the page initializes; after switching rules or changing the type, update the matching Scene ID and reload the page.',
