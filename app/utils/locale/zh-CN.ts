...
export const siteConfig = {
  ...
  esaCaptchaEndpointRegister: '注册',
  esaCaptchaHost: '域名',
  esaCaptchaHostPlaceholder: '* 或 hub.example.com 或 *.example.com',
  esaCaptchaKeepOne: '至少保留一条场景配置',
  esaCaptchaRegion: '部署区域',
  esaCaptchaRegionCn: '中国内地',
  esaCaptchaRegionSgp: '新加坡',
  esaCaptchaDesc: '开启后，所有用户在每次登录与注册时都需要通过 ESA AI 验证码验证。ESA 侧一条规则只对应一个接口加一个域名，需先在 ESA 控制台为登录接口、注册接口以及每个对外域名分别新增规则，再把各自的场景 ID 登记到下表。验证码形态（无痕/一点即过/滑块/拼图等）由场景 ID 所属的那条规则决定，并在页面初始化时由 ESA 下发；更换规则或修改形态后，请同步更新对应的场景 ID 并刷新页面。验签在 ESA 边缘完成，源站只确认验签请求头是否存在，因此启用前必须确保源站只接受来自 ESA 回源的流量（防火墙或安全组仅放行 ESA 回源网段，禁止公网直连源站端口），否则人机验证可被绕过。',
...
export const pages = {
  ...
  account: {
    ...
    social: {
      ...
      emailUnbound: '邮箱已解绑',
      astrbot: {
        platforms: { qq: 'QQ', wecom: '企业微信', dingtalk: '钉钉', lark: '飞书' },
        desc: '生成一次性绑定码后，将其私发给对应平台的机器人完成绑定。',
        bound: '已绑定',
        boundAt: '绑定时间：',
        generate: '生成绑定码',
        instruction: '请将下方绑定码私发给对应平台的机器人，不要发到群聊。',
        expires: '绑定码在 {0} 秒后失效；每个平台各自保留一个绑定码，重新生成会替换该平台的旧码。',
        refresh: '我已发送，刷新绑定状态',
        test: '发送测试通知',
        testSuccess: '测试通知已发送',
        testQueued: '测试通知已加入队列，请等待机器人领取；这不代表已送达。',
        testFailed: '发送测试通知失败',
        unbind: '解除绑定',
        unbindTitle: '解除平台绑定',
        unbindMessage: '解绑 {0} 后将不再收到该平台通知，确定继续吗？',
        unbound: '平台已解绑',
        unbindFailed: '解绑失败',
        codeFailed: '生成绑定码失败',
        loadFailed: '加载平台绑定状态失败'
      }
    },
    personalApiKey: {
      title: '个人 API Key',
  ...
  auth: {
    loginForm: {
      ...
      esaCaptchaSceneMissing: '当前接口尚未配置 ESA 验证码场景 ID，请联系管理员',
      esaCaptchaLoadFailed: '人机验证组件加载失败，请刷新页面重试',
      esaVerifyFailed: '人机验证未通过（原因码 {0}），请重试或联系管理员',
      esaVerifyFailedWithReason: '人机验证未通过：{0}（原因码 {1}），请重试或联系管理员',
      // ESA 边缘回传的验签原因码说明，未收录的码只展示码本身
      esaVerifyCodes: {
        F001: '疑似攻击请求，风险策略不通过',
        F003: '验签参数解析错误',
        F004: 'ESA 控制台开启测试模式且配置为验证不通过',
        F005: '场景 ID 不存在',
        F008: '验证数据重复提交',
        F009: '检测到虚拟设备或模拟移动设备环境',
        F010: '同一 IP 访问频率超出限制',
        F011: '同一设备访问频率超出限制',
        F014: '无初始化记录，或初始化与验证间隔超出 20 分钟',
        F015: '验证交互不通过',
        F016: '验证码规则配置的接口与实际请求不匹配',
        F017: '疑似攻击请求，协议或参数异常',
        F018: '验签数据重复使用',
        F019: '验签超出 90 秒有效期，或未发起验证就验签',
        F020: '验签票据与场景 ID 或用户不匹配',
        F021: '验证与验签使用的场景 ID 不一致',
        F022: '客户端接入异常，资源防盗校验失败',
        F024: '检测到自动化脚本模拟点击或滑动'
      },
      ...
      legalConsentBlocked: '请先阅读并同意最新条款后再登录',
      legalConsentRelatedDocs: '相关文档',
      legalConsentModalTitle: '条款更新通知',
      legalConsentModalDesc: '请仔细阅读以下条款，同意后进入系统。',
...
export const admin = {
  ...
  astrbotManager: {
    title: '机器人通知配置',
    desc: '统一配置 QQ、企业微信、钉钉和飞书的账户通知；通知只发送到已绑定并启用的私聊会话。',
    enabled: '启用机器人通知总开关',
    platformHint: '启用的平台会显示在用户的账户绑定页面。',
    platforms: { qq: '启用 QQ', wecom: '启用企业微信', dingtalk: '启用钉钉', lark: '启用飞书' },
    ...
    pushMode: '推送方向',
    pushModePush: 'push（VoiceHub 主动推送）',
    pushModePull: 'pull（插件主动拉取）',
    pushModeHint: 'push 要求 VoiceHub 能访问插件端口；插件在内网/NAT 后时选 pull，由插件轮询取件，需在插件侧同时设置拉取间隔。',
    weeklyTitle: '本周歌单图片显示项',
    weeklyHint: '用于 /广播 本周歌单 的图片；与此浏览器本地保存的打印排期导出方案相互独立。',
    weeklyFields: { showCover: '显示封面', showSequence: '显示序号', showRequester: '显示投稿人', showVotes: '显示票数', showPlayTime: '显示播出时段', showDate: '显示日期' },
    groupTitle: '群聊推送',
    groupHint: '把点歌投稿、注册待审核、备份失败与系统异常推送到指定群。群目标必须逐条标注所属平台：会话串的前缀是 AstrBot 平台实例 ID，无法自动判断平台。',
    groupEnabled: '启用群聊推送',
    groupTargetsTitle: '群目标白名单',
    groupTargetsHint: '会话串在群里发送 /广播 状态 获取，形如 default:GroupMessage:123456。未列入白名单的群不会被推送。',
    groupTargetPlaceholder: 'default:GroupMessage:123456',
    groupLabelPlaceholder: '备注（可选）',
    groupAdd: '添加群目标',
    groupRemove: '删除',
    groupEventsTitle: '推送哪些事件',
    groupEvents: {
      songRequest: '新点歌投稿（谁点了什么歌）',
    ...
    groupMergeWindow: '合并窗口（秒）',
    groupMinInterval: '同群同事件最短间隔（秒）',
    groupThrottleHint: '同一窗口内的高频事件会合并成一条消息，避免刷屏；留空或填 0 表示不合并、立即发送。',
    save: '保存配置',
    saving: '保存中...',
    saved: 'AstrBot 配置已保存',
...
export const serverErrors = {
  ...
  SETTINGS_CAPTCHA_PROVIDER_INVALID: '验证类型必须是图形验证码、Cloudflare Turnstile 或阿里云 ESA AI 验证码',
  ...
  ASTRBOT_UMO_UNBOUND: '机器人会话尚未绑定账号',
  ASTRBOT_SONG_KEYWORD_INVALID: '点歌关键词无效',
