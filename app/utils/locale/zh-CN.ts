...
export const siteConfig = {
  ...
  esaCaptchaDesc: '开启后，所有用户在每次登录与注册时都需要通过 ESA AI 验证码验证。ESA 侧一条规则只对应一个接口加一个域名，需先在 ESA 控制台为登录接口、注册接口以及每个对外域名分别新增规则，再把各自的场景 ID 登记到下表。验证码形态（无痕/一点即过/滑块/拼图等）由场景 ID 所属的那条规则决定，并在页面初始化时由 ESA 下发；更换规则或修改形态后，请同步更新对应的场景 ID 并刷新页面。',
