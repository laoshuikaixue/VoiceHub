...
export const siteConfig = {
  ...
  esaCaptchaSceneIdPlaceholder: '在此输入验证码规则列表中该规则的场景 ID',
  esaCaptchaRegion: '部署区域',
  esaCaptchaRegionCn: '中国内地',
  esaCaptchaRegionSgp: '新加坡',
  esaCaptchaDesc: '开启后，所有用户在每次登录时都需要在 ESA AI 验证码完成验证；需先在 ESA 控制台新增验证码规则，并把本站登录接口配置为需验签的接口。验证码形态（无痕/一点即过/滑块/拼图等）由场景 ID 所属的那条规则决定，并在登录页初始化时由 ESA 下发；更换规则或修改形态后，请同步更新场景 ID 并刷新登录页。',
...
export const serverErrors = {
  ...
  SETTINGS_ESA_CAPTCHA_CREDENTIALS_MISSING: '请先配置阿里云 ESA AI 验证码的身份标和场景 ID，再切换到该验证类型',
