    ...
    >
      <div v-if="show" class="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" @click.self="close">
        <div class="bg-zinc-900 border border-zinc-800 w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
          ...
          <!-- 结果展示视图 -->
          <div v-if="importResult" class="flex flex-col flex-1 overflow-hidden animate-in fade-in duration-500">
            ...
            <div class="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
              <div class="grid grid-cols-2 gap-4">
                <div class="p-6 bg-emerald-500/5 border border-emerald-500/20 rounded-3xl flex flex-col items-center gap-2">
                  <span class="text-[10px] font-black text-emerald-500 uppercase tracking-widest">成功导入</span>
                  <span class="text-4xl font-black text-emerald-400">{{ importResult.success }}</span>
                </div>
                <div class="p-6 bg-red-500/5 border border-red-500/20 rounded-3xl flex flex-col items-center gap-2">
                  <span class="text-[10px] font-black text-red-500 uppercase tracking-widest">失败/跳过</span>
              ...
              <div v-if="importResult.details && importResult.details.length > 0" class="space-y-4">
                <div class="flex items-center gap-2 text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">
                  <Icon name="info" :size="12" />
                  详细处理记录
                </div>
                <div class="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                  <div v-for="(detail, index) in importResult.details" :key="index" class="p-3 bg-zinc-950 border border-zinc-800 rounded-xl text-xs text-zinc-400 font-medium">
                    {{ detail }}
                  </div>
                </div>
              </div>
              <div v-else class="flex flex-col items-center justify-center py-10 text-emerald-500/50">
            ...
            <div class="p-8 pt-4 border-t border-zinc-800/50 bg-zinc-900/50">
              <button class="w-full px-6 py-4 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs font-black rounded-2xl transition-all uppercase tracking-widest shadow-lg active:scale-95" @click="close">
                完成并返回
              </button>
            </div>
          </div>
          ...
          <div v-else class="flex flex-col flex-1 overflow-hidden">
            ...
            <div class="p-8 py-6 space-y-6 border-b border-zinc-800/50">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
                <div class="space-y-2">
                  <label class="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">选择学期</label>
                  <CustomSelect
                ...
                <div class="flex bg-zinc-950 p-1 rounded-xl border border-zinc-800">
                  <button 
                    ...
                    :key="type"
                    type="button"
                    :class="['flex-1 px-4 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all', 
                      filterType === type ? 'bg-blue-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300']"
            ...
            <div class="flex-1 overflow-y-auto p-4 custom-scrollbar bg-zinc-950/30">
              <div v-if="loadingSemesters || loadingSongs" class="flex flex-col items-center justify-center py-20 text-zinc-500">
              ...
              <div v-else-if="error" class="flex flex-col items-center justify-center py-20 text-center px-8">
                ...
                <button class="px-6 py-3 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-black rounded-xl transition-all uppercase tracking-widest" @click="loadSongs">
                  重试加载
                </button>
              </div>

              <div v-else-if="!selectedSemester" class="flex flex-col items-center justify-center py-20 text-zinc-600">
                <Icon name="list" :size="48" class="mb-4 opacity-20" />
                <p class="text-sm font-bold uppercase tracking-widest">请先选择一个学期</p>
              </div>

              <div v-else-if="filteredSongs.length === 0" class="flex flex-col items-center justify-center py-20 text-zinc-600">
              ...
              <div v-else class="grid grid-cols-1 gap-2">
                <div 
                  ...
                  :class="[isSelected(song.id) ? 'bg-blue-600/10 border-blue-500/30 shadow-lg' : 'bg-zinc-900 border-transparent hover:border-zinc-800']"
                  @click="toggleSelection(song.id)"
                >
                  <div class="w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all"
                    :class="[isSelected(song.id) ? 'bg-blue-600 border-blue-600 text-white' : 'border-zinc-800 text-transparent']"
                  >
                    <Icon name="check" :size="14" />
                  </div>

                  <div class="w-12 h-12 rounded-xl overflow-hidden bg-zinc-800 flex-shrink-0 shadow-md">
                    <img :src="convertToHttps(song.cover)" alt="cover" class="w-full h-full object-cover" loading="lazy" />
                  </div>

                  <div class="flex-1 min-w-0">
                    <h4 class="text-sm font-black text-zinc-100 truncate mb-0.5">{{ song.title }}</h4>
            ...
            <div class="p-8 border-t border-zinc-800/50 bg-zinc-900/50 flex flex-col md:flex-row gap-4 items-center justify-between">
              <div class="flex items-center gap-4 w-full md:w-auto">
                <button type="button" class="text-[10px] font-black text-blue-500 hover:text-blue-400 uppercase tracking-widest transition-colors" @click="toggleSelectAll">
              ...
              <div class="flex gap-3 w-full md:w-auto">
                <button type="button" class="flex-1 md:flex-none px-6 py-4 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs font-black rounded-2xl transition-all uppercase tracking-widest" @click="close">
                  取消
                </button>
                <button 
                  type="button"
...
const fetchSemesters = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      semesterList.value = res.data.filter(sem => sem.isActive === false || sem.isActive === 0 || sem.isActive === '0')
      // 不自动选择，强制用户手动选择
    }
  } catch (e) {
