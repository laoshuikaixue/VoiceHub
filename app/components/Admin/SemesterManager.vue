    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
      ...
      <div class="lg:col-span-5 space-y-6">
        <div class="flex items-center gap-2 px-1">
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-amber-500 fill-amber-500"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>
        ...
        <div v-if="currentSemester" class="relative bg-gradient-to-br from-blue-600 to-blue-700 rounded-3xl p-10 shadow-2xl shadow-blue-900/20 overflow-hidden group">
          <div class="relative z-10 space-y-6">
            ...
            <div class="pt-4">
              <span class="px-4 py-2 bg-white text-blue-600 text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg">
                正在进行中
              </span>
            </div>
          </div>
          ...
          <!-- 装饰性背景元素 -->
          <div class="absolute -right-10 -bottom-10 w-64 h-64 bg-white/5 blur-3xl rounded-full"></div>
          <div class="absolute top-10 right-10 opacity-10">
            <svg xmlns="http://www.w3.org/2000/svg" width="160" height="160" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-white"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"></path><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"></path></svg>
          </div>
        </div>
        ...
        <div class="p-6 bg-zinc-900/20 border border-zinc-800 rounded-3xl flex items-start gap-4">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-zinc-500 shrink-0 mt-0.5"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>
          <div class="space-y-1">
      ...
      <div class="lg:col-span-7 space-y-6">
        <div class="flex items-center justify-between px-1">
          <div class="flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-zinc-600"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"></path><path d="M3 3v5h5"></path><path d="M12 7v5l4 2"></path></svg>
            <h3 class="text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em]">学期历史记录</h3>
          </div>
          <span class="text-[10px] font-black text-zinc-700">共 {{ semesters.length }} 条</span>
        </div>

        <div class="space-y-4">
          <div v-if="loading && semesters.length === 0" class="text-center py-20 bg-zinc-900/10 border border-zinc-800 border-dashed rounded-[2rem]">
          ...
          <div 
            ...
            :class="sem.isActive ? 'bg-blue-600/5 border-blue-500/30' : 'bg-zinc-900/40 border-zinc-800 hover:border-zinc-700'"
          >
            <div class="flex items-center gap-5">
              <div 
                class="w-12 h-12 rounded-2xl flex items-center justify-center transition-all"
                :class="sem.isActive ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20' : 'bg-zinc-800 text-zinc-600 border border-zinc-700'"
              >
              ...
              <div>
                <h5 
                  ...
                  :class="sem.isActive ? 'text-blue-400' : 'text-zinc-200 group-hover:text-blue-400'"
                >
                  {{ sem.name }}
                </h5>
            ...
            <div class="flex items-center gap-2">
              <div v-if="sem.isActive" class="flex items-center gap-2 px-4 py-2 bg-emerald-500/10 text-emerald-500 rounded-xl border border-emerald-500/20">
          ...
          <div v-if="semesters.length === 0 && !loading" class="text-center py-20 bg-zinc-900/10 border border-zinc-800 border-dashed rounded-[2rem]">
      ...
      <div class="w-full max-w-md bg-zinc-900 rounded-3xl border border-zinc-800 shadow-2xl overflow-hidden" @click.stop>
        <div class="p-8 space-y-6">
          <div class="flex items-center justify-between">
            ...
            <button @click="closeAddModal" class="p-2 text-zinc-500 hover:text-white transition-colors">
          ...
          <div class="space-y-6">
            <div class="space-y-2">
              <label class="text-[10px] font-black text-zinc-600 uppercase tracking-widest px-1">学期名称</label>
              <input 
            ...
            <label class="flex items-center gap-3 cursor-pointer group px-1">
              ...
              />
              <div>
                <span class="text-xs font-bold text-zinc-300 group-hover:text-blue-400 transition-colors">设为当前活跃学期</span>
                <p class="text-[10px] text-zinc-600 font-medium">创建后将立即激活此学期，原学期将变为历史状态</p>
              </div>
            </label>

            <div class="p-4 bg-zinc-950 border border-zinc-800 rounded-2xl flex items-start gap-3">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-amber-500 shrink-0 mt-0.5"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
        ...
        <div class="p-6 bg-zinc-800/50 border-t border-zinc-800 flex gap-3 justify-end">
          <button @click="closeAddModal" class="px-6 py-2.5 text-xs font-bold text-zinc-500 hover:text-zinc-300 transition-colors">取消</button>
          <button 
