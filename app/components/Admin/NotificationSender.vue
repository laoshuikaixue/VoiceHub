  ...
  <div class="max-w-[1400px] mx-auto space-y-8 pb-20">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div class="lg:col-span-7 space-y-6">
        <div class="bg-zinc-900/30 border border-zinc-800 rounded-[2rem] p-8 shadow-xl">
          <div class="space-y-6">
            <!-- 标题 -->
            <div class="space-y-2">
              <label class="text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em] px-1">通知标题</label>
              <input 
                type="text" 
                v-model="form.title"
                placeholder="请输入通知标题" 
            ...
            <!-- 内容 -->
            <div class="space-y-2">
              <label class="text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em] px-1">通知内容</label>
              <textarea 
            ...
            <div class="space-y-3 pt-4 border-t border-zinc-800/50">
              <label class="text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em] px-1">通知范围</label>
            ...
            <div class="pt-2">
              <Transition name="fade" mode="out-in">
                <div v-if="form.scope === 'ALL'" key="all" class="p-4 bg-blue-500/5 border border-blue-500/10 rounded-2xl flex items-center gap-4">
                  <Info class="text-blue-500 shrink-0" :size="18" />
                  <p class="text-[11px] font-bold text-zinc-400">将向系统中所有已注册的活跃用户发送此通知。</p>
                </div>
                ...
                <div v-else-if="form.scope === 'CLASS'" key="class" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="space-y-1.5">
                  ...
                  <div class="space-y-1.5">
                    <span class="text-[9px] font-black text-zinc-600 uppercase tracking-widest px-1">班级</span>
                    <input 
                      type="text" 
                ...
                <div v-else-if="form.scope === 'MULTI_CLASS'" key="multi" class="space-y-4">
                  <div class="p-6 bg-zinc-950/50 border border-zinc-800 border-dashed rounded-2xl">
                    ...
                    <div v-if="form.selectedClasses.length > 0" class="flex flex-wrap gap-2">
                      ...
                      >
                        <span class="text-[10px] font-bold text-zinc-400">{{ cls.grade }} {{ cls.class }}</span>
                        <button @click="removeClassFromSelection(index)" class="text-zinc-600 hover:text-red-400 transition-colors">
                          <X :size="12" />
                        </button>
                      </div>
                    </div>
                    <div v-else class="text-center py-4">
                      <p class="text-[10px] font-black text-zinc-700 uppercase tracking-widest">未选择任何班级</p>
                    </div>
                  </div>
                </div>
                ...
                <div v-else-if="form.scope === 'SPECIFIC_USERS'" key="specific" class="space-y-4">
                  ...
                  <!-- 搜索结果 -->
                  <div v-if="showUserSearchResults && userSearchResults.length > 0" class="bg-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden max-h-[300px] overflow-y-auto custom-scrollbar">
                    <div class="px-4 py-2 border-b border-zinc-800/60 bg-zinc-900/40">
                      <span class="text-[9px] font-black text-zinc-600 uppercase tracking-widest">搜索结果 ({{ userSearchResults.length }})</span>
                    </div>
                    <div v-for="user in userSearchResults" :key="user.id" class="flex items-center justify-between p-4 border-b border-zinc-800/30 last:border-0 hover:bg-zinc-800/30 transition-all">
                      <div>
                        <h5 class="text-sm font-bold text-zinc-200">{{ user.name || user.username }}</h5>
                        <div class="flex items-center gap-2 mt-0.5">
                          <span class="text-[10px] text-blue-500 font-black">@{{ user.username }}</span>
                          <span v-if="user.grade && user.class" class="text-[10px] text-zinc-600 font-bold uppercase tracking-widest">{{ user.grade }} {{ user.class }}</span>
                          <span class="text-[10px] text-zinc-700 font-black">{{ getRoleText(user.role) }}</span>
                        </div>
                      </div>
                      <button 
                  ...
                  <div v-if="form.selectedUsers.length > 0" class="space-y-3">
                    <div class="flex items-center justify-between px-1">
                      <span class="text-[9px] font-black text-zinc-600 uppercase tracking-widest">已选择用户 ({{ form.selectedUsers.length }})</span>
                      <button @click="clearAllSelectedUsers" class="text-[9px] font-black text-red-500/70 hover:text-red-500 uppercase tracking-widest transition-colors">清空全部</button>
                    </div>
                    <div class="flex flex-wrap gap-2">
                      <div 
                      ...
                      >
                        <div class="flex flex-col">
                          <span class="text-[10px] font-bold text-zinc-300 leading-none">{{ user.name || user.username }}</span>
                          <span class="text-[8px] text-zinc-600">@{{ user.username }}</span>
                        </div>
                        <button @click="removeUserFromSelection(index)" class="text-zinc-600 hover:text-red-400 transition-colors">
                          <X :size="12" />
                        </button>
                      </div>
                    </div>
                  ...
                  <div v-else class="text-center py-4">
                    <p class="text-[10px] font-black text-zinc-700 uppercase tracking-widest">未选择任何用户</p>
                  </div>
                </div>
              </Transition>
            </div>
          ...
          <div v-if="error || success" class="mt-6">
            <div v-if="error" class="p-4 bg-red-500/5 border border-red-500/10 rounded-2xl flex items-center gap-3">
              <AlertCircle class="text-red-500 shrink-0" :size="18" />
              <p class="text-xs font-bold text-red-400">{{ error }}</p>
            </div>
            <div v-if="success" class="p-4 bg-green-500/5 border border-green-500/10 rounded-2xl flex items-center gap-3">
      ...
      <div class="lg:col-span-5 space-y-6 lg:sticky lg:top-8">
        <div class="bg-zinc-900/30 border border-zinc-800 rounded-[2rem] p-8 shadow-xl flex flex-col h-full">
          <h3 class="text-sm font-black text-zinc-100 uppercase tracking-widest mb-6 flex items-center gap-2">
            <Eye :size="16" class="text-blue-500" /> 通知预览
          </h3>
          
          <div class="flex-1 flex flex-col items-center justify-center p-4">
            <div class="w-full max-w-[320px] bg-[#0c0c0e] border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl relative">
              <!-- 顶部装饰 -->
              ...
              <div class="p-6 space-y-4">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="w-8 h-8 rounded-xl bg-blue-600/10 text-blue-500 flex items-center justify-center border border-blue-500/20">
                      <Bell :size="14" />
                    </div>
                    <span class="text-[10px] font-black text-zinc-600 uppercase tracking-widest">VoiceHub 系统</span>
                  </div>
                  <span class="text-[9px] text-zinc-700 font-bold uppercase tracking-wider">刚刚</span>
                </div>

                <div class="space-y-2">
                  <h4 :class="['text-sm font-black transition-colors', form.title ? 'text-zinc-100' : 'text-zinc-800 italic']">
                    {{ form.title || '通知标题' }}
                  </h4>
                  <p :class="['text-[11px] leading-relaxed transition-colors', form.content ? 'text-zinc-400' : 'text-zinc-800 italic line-clamp-3']">
                ...
                <div class="pt-4 border-t border-zinc-800/50 flex items-center justify-between">
                  ...
                  <button class="p-1.5 text-blue-500 hover:bg-blue-600/10 rounded-lg transition-all">
              ...
              <!-- 背景光晕 -->
              <div class="absolute top-0 right-0 w-32 h-32 bg-blue-600/5 blur-[50px] -z-10 rounded-full" />
            </div>

            <div class="mt-8 space-y-3 w-full max-w-[320px]">
               <div class="flex items-start gap-3 p-3 bg-amber-500/5 border border-amber-500/10 rounded-2xl">
...
import {
  Send, Users, GraduationCap, LayoutGrid, User,
