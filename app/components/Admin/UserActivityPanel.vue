<template>
  <div class="user-activity-panel">
    ...
    <template v-if="!forbidden && data">
      ...
      <OpsPanel title="在线会话列表" subtitle="逐条查看账号、角色、来源、设备与最后活动时间；必要时可强制下线。" :status="panelStatus" :updated-at="updatedAt" :pending="loading && !data" :refreshing="loading && Boolean(data)" :error="Boolean(error)" :empty="!sessions.length && !loading" empty-text="当前筛选范围暂无统计数据。" @refresh="loadSessions(page)">
...
const recommendations = computed(() => {
  ...
  if (unknownClients) items.push({ icon: 'warning', status: 'unknown', title: '识别未知客户端', detail: `有 ${unknownClients} 个会话无法识别浏览器，可结合 IP 地址与最近页面继续排查。`, tag: '信息不足' })
...
.activity-kpis article { --activity-accent: var(--ops-info); display: flex; min-height: 7.5rem; flex-direction: column; border: 1px solid var(--ops-line); border-radius: 6px; padding: .8rem; background: var(--ops-control); }
...
.browser-distribution, .device-distribution { min-width: 0; border: 1px solid var(--ops-line); border-radius: 6px; padding: .8rem; background: var(--ops-control); }
...
.browser-distribution__content { display: grid; grid-template-columns: 6.5rem minmax(0, 1fr); align-items: center; gap: .8rem; margin-top: 1rem; }.browser-donut { display: grid; width: 6rem; height: 6rem; place-items: center; border-radius: 50%; }.browser-donut > span { display: flex; width: 4.3rem; height: 4.3rem; flex-direction: column; align-items: center; justify-content: center; border-radius: 50%; background: var(--ops-control); }.browser-donut strong { font-family: var(--ops-mono); font-size: 1rem; }.browser-donut small { color: var(--ops-text-2); font-size: .6rem; }.browser-distribution ol { display: grid; gap: .5rem; margin: 0; padding: 0; list-style: none; }.browser-distribution li { display: grid; grid-template-columns: .45rem minmax(0, 1fr) auto auto; align-items: center; gap: .4rem; font-size: .65rem; }.browser-distribution li i { width: .4rem; height: .4rem; border-radius: 50%; }.browser-distribution li span, .browser-distribution li small { color: var(--ops-text-2); }.browser-distribution li strong { font-family: var(--ops-mono); }
...
.activity-recommendations { display: grid; gap: .65rem; }.activity-recommendations article { display: grid; grid-template-columns: 2rem minmax(0, 1fr) auto; align-items: center; gap: .7rem; border: 1px solid var(--ops-line); border-radius: 6px; padding: .7rem; background: var(--ops-control); }.activity-recommendations article > span { display: inline-flex; width: 2rem; height: 2rem; align-items: center; justify-content: center; border-radius: 4px; color: var(--ops-unknown); background: color-mix(in srgb, var(--ops-text-2) 8%, transparent); }.activity-recommendations strong { font-size: .72rem; }.activity-recommendations p { margin: .3rem 0 0; color: var(--ops-text-2); font-size: .64rem; line-height: 1.5; }.activity-recommendations em { border: 1px solid var(--ops-line); border-radius: 4px; padding: .22rem .35rem; color: var(--ops-text-2); font-size: .6rem; font-style: normal; }.activity-recommendation--warning > span { color: var(--ops-warning) !important; background: color-mix(in srgb,var(--ops-warning) 8%,transparent) !important; }.activity-recommendation--warning em { border-color: color-mix(in srgb,var(--ops-warning) 30%,transparent); color: var(--ops-warning); }.activity-recommendation--ok > span { color: var(--ops-ok) !important; }
.activity-filters { display: grid; grid-template-columns: minmax(0, 1fr); gap: .65rem; border: 1px solid var(--ops-line); border-radius: 6px; padding: .75rem; background: var(--ops-panel); }.activity-filters label { display: grid; grid-template-columns: 4.5rem minmax(0, 1fr); align-items: center; gap: .5rem; color: var(--ops-text-2); font-size: .6875rem; }.activity-filters input { width: 100%; height: 1.875rem; border: 1px solid var(--ops-line); border-radius: 6px; padding: 0 .6rem; outline: none; color: var(--ops-text-1); background: var(--ops-control); font-size: .7rem; }.activity-filters input:focus { border-color: color-mix(in srgb, var(--ops-info) 50%, var(--ops-line)); }.activity-select { min-width: 0; }.activity-filter-actions { display: flex; justify-content: flex-end; gap: .5rem; }.activity-filter-actions button { height: 1.875rem; border: 1px solid var(--ops-line); border-radius: 6px; padding: 0 .75rem; color: var(--ops-text-1); background: var(--ops-control); font-size: .7rem; cursor: pointer; }.activity-filter-actions button:hover { border-color: color-mix(in srgb, var(--ops-info) 50%, var(--ops-line)); }.activity-filter-actions .activity-button--primary { border-color: color-mix(in srgb, var(--ops-info) 50%, var(--ops-line)); color: var(--ops-info); }
:deep(.activity-select > div) { min-height: 1.875rem; height: 1.875rem; border-color: var(--ops-line); border-radius: 6px; padding: 0 .6rem; background: var(--ops-control); }:deep(.activity-select > div:hover) { border-color: color-mix(in srgb, var(--ops-info) 50%, var(--ops-line)); }
.activity-table-wrap { overflow-x: auto; }.activity-table { width: 100%; min-width: 1050px; border-collapse: collapse; }.activity-table th { border-bottom: 1px solid var(--ops-line); padding: .65rem .55rem; color: var(--ops-text-2); font-size: .64rem; font-weight: 600; text-align: left; white-space: nowrap; }.activity-table td { max-width: 12rem; border-bottom: 1px solid var(--ops-line-soft); padding: .65rem .55rem; color: var(--ops-text-1); font-size: .68rem; }.activity-table td > strong, .activity-table td > small { display: block; }.activity-table td > small { margin-top: .2rem; color: var(--ops-text-2); }.activity-mono { font-family: var(--ops-mono); }.activity-path { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.activity-session-status { display: inline-flex; border: 1px solid var(--ops-line); border-radius: 4px; padding: .2rem .35rem; color: var(--ops-text-2); }.activity-session-status--active { border-color: color-mix(in srgb,var(--ops-ok) 30%,transparent); color: var(--ops-ok); background: color-mix(in srgb,var(--ops-ok) 8%,transparent); }.activity-session-status--idle { border-color: color-mix(in srgb,var(--ops-warning) 30%,transparent); color: var(--ops-warning); background: color-mix(in srgb,var(--ops-warning) 8%,transparent); }.activity-revoke { display: inline-flex; height: 1.75rem; align-items: center; gap: .3rem; border: 1px solid color-mix(in srgb,var(--ops-error) 30%,transparent); border-radius: 5px; padding: 0 .45rem; color: var(--ops-error); background: color-mix(in srgb,var(--ops-error) 7%,var(--ops-control)); font-size: .64rem; cursor: pointer; }.activity-revoke:disabled { cursor: not-allowed; opacity: .5; }
.activity-table-actions { display: flex; align-items: center; gap: .4rem; white-space: nowrap; }.activity-details-toggle { height: 1.75rem; border: 1px solid var(--ops-line); border-radius: 5px; padding: 0 .45rem; color: var(--ops-info); background: var(--ops-control); font-size: .64rem; cursor: pointer; }.activity-details-toggle:hover { border-color: color-mix(in srgb,var(--ops-info) 55%,transparent); }.activity-table__row--expanded > td { border-bottom-color: transparent; }.activity-session-detail-row > td { padding: 0 .55rem .75rem; border-bottom: 1px solid var(--ops-line-soft); }.activity-session-detail { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); overflow: hidden; margin: 0; border: 1px solid var(--ops-line); border-radius: 6px; background: var(--ops-control); }.activity-session-detail > div { display: grid; grid-template-columns: 6.25rem minmax(0, 1fr); min-height: 2.25rem; border-right: 1px solid var(--ops-line-soft); border-bottom: 1px solid var(--ops-line-soft); }.activity-session-detail > div:nth-child(2n) { border-right: 0; }.activity-session-detail > div:nth-last-child(-n + 2) { border-bottom: 0; }.activity-session-detail dt { display: flex; align-items: center; padding: .45rem .55rem; color: var(--ops-text-2); background: color-mix(in srgb, var(--ops-text-2) 4.5%, transparent); font-size: .66rem; font-weight: 600; }.activity-session-detail dd { min-width: 0; margin: 0; padding: .45rem .55rem; color: var(--ops-text-1); font-size: .68rem; line-height: 1.45; word-break: break-word; }.activity-session-detail__wide { grid-column: 1 / -1; border-right: 0 !important; }.activity-user-agent { color: var(--ops-text-2) !important; font-family: var(--ops-mono); }.activity-detail-path { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.activity-pagination { display: flex; align-items: center; justify-content: space-between; gap: .75rem; margin-top: .75rem; color: var(--ops-text-2); font-size: .68rem; }.activity-pagination div { display: flex; align-items: center; gap: .5rem; }.activity-pagination button { height: 1.75rem; border: 1px solid var(--ops-line); border-radius: 5px; padding: 0 .5rem; color: var(--ops-text-1); background: var(--ops-control); font-size: .65rem; cursor: pointer; }.activity-pagination button:disabled { cursor: not-allowed; opacity: .4; }.activity-pagination strong { color: var(--ops-text-1); font-family: var(--ops-mono); }
