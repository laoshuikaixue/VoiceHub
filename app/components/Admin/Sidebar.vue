      ...
      <div class="flex items-center px-2 mb-6 mt-2">
        <NuxtLink to="/" class="flex items-center gap-2.5 group">
          ...
          <div class="flex flex-col justify-center">
            ...
            <p class="text-[10px] text-zinc-500 mt-1.5 uppercase tracking-widest font-bold leading-none">管理控制台</p>
          </div>
        </NuxtLink>
      </div>
            ...
            <template v-for="item in group.items" :key="item.id">
              ...
              >
                ...
                <component 
                  :is="item.icon" 
                  :size="18" 
                  :class="activeTab === item.id ? 'text-blue-400' : 'text-zinc-500 group-hover:text-zinc-300'" 
                />
                <span class="truncate">{{ item.label }}</span>
                <!-- 选中状态指示器 -->
                <div v-if="activeTab === item.id" class="ml-auto w-1 h-1 bg-blue-400 rounded-full shadow-[0_0_8px_rgba(96,165,250,0.6)]"></div>
              </button>
            </template>
          </template>
        </div>
      ...
      <div class="mt-4 pt-4 border-t border-zinc-800">
        <div class="flex items-center gap-3 p-3 rounded-lg bg-zinc-900/50 border border-zinc-800/50 hover:bg-zinc-800/30 transition-colors">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-xs font-black truncate text-zinc-100">{{ currentUser?.name || '管理员' }}</p>
            <p class="text-[10px] text-zinc-500 truncate uppercase tracking-wider font-medium mt-0.5">
