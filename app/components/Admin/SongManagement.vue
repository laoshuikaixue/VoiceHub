<template>
  <div class="space-y-10 max-w-[1600px] mx-auto pb-20">
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          >
            <div
              ...
              class="hidden sm:flex flex-row items-center gap-2 p-1.5 bg-zinc-900 bg-opacity-90 border border-zinc-800 rounded-lg backdrop-blur-xl shadow-2xl"
            >
              <button
                class="flex items-center justify-center gap-2 px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-emerald-400 text-[11px] font-black rounded-lg border border-zinc-700 border-opacity-50 transition-all active:scale-95 uppercase tracking-widest"
                @click="openDownloadDialog"
              >
                <Download :size="14" class="text-emerald-500 text-opacity-70" /> {{ locale.actions.download }} ({{
                  selectedSongs.length
                }})
              </button>
              <button
                class="flex items-center justify-center gap-2 px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-red-400 text-[11px] font-black rounded-lg border border-zinc-700 border-opacity-50 transition-all active:scale-95 uppercase tracking-widest"
                @click="batchDelete"
              >
                <Trash2 :size="14" class="text-red-500 text-opacity-70" /> {{ locale.actions.delete }} ({{ selectedSongs.length }})
              </button>
            </div>
          </Transition>
          ...
          <button
            :disabled="loading"
            class="flex items-center gap-2 px-5 py-2.5 bg-blue-600 bg-opacity-10 hover:bg-blue-600 bg-opacity-20 text-blue-400 text-xs font-bold rounded-lg border border-blue-500 border-opacity-20 transition-all disabled:opacity-50"
      ...
      <!-- 过滤器栏 -->
      <div
        class="bg-zinc-900 bg-opacity-30 border border-zinc-800 border-opacity-60 rounded-xl p-4 flex flex-col lg:flex-row gap-4 items-stretch lg:items-center"
      >
        <div class="relative flex-1 group w-full lg:w-auto">
          ...
          <input
            ...
            class="w-full bg-zinc-950 bg-opacity-50 border border-zinc-800 border-opacity-80 rounded-lg pl-12 pr-4 py-3 text-sm focus:outline-none focus:border-blue-500 border-opacity-30 transition-all placeholder:text-zinc-800 text-zinc-200"
          />
        </div>
    ...
    <!-- 歌曲表格 -->
    <div
      class="bg-zinc-900 bg-opacity-10 border border-zinc-800 border-opacity-40 rounded-xl overflow-hidden shadow-2xl relative"
    >
      <!-- 加载遮罩 -->
      <div
        v-if="loading"
        class="absolute inset-0 bg-zinc-950 bg-opacity-40 backdrop-blur-sm z-10 flex items-center justify-center"
      >
        <div class="flex flex-col items-center gap-3">
          <div
            class="w-8 h-8 border-2 border-blue-500 border-opacity-20 border-t-blue-500 rounded-full animate-spin"
          />
      ...
      <!-- 表头 -->
      <div
        class="hidden lg:grid grid-cols-12 gap-4 px-8 py-5 border-b border-zinc-800 border-opacity-60 bg-zinc-900 bg-opacity-40"
      >
      ...
      <div
        ...
        class="lg:hidden flex items-center justify-between px-6 py-3 border-b border-zinc-800 border-opacity-60 bg-zinc-900 bg-opacity-40"
      >
      ...
      <div v-else class="divide-y divide-zinc-800/40">
        <div
          ...
          :class="[
            ...
            selectedSongs.includes(song.id) ? 'bg-blue-600 bg-opacity-5' : 'hover:bg-zinc-800 bg-opacity-20'
          ]"
        >
          ...
          <div class="col-span-12 lg:col-span-4 flex items-center gap-3 lg:gap-4 w-full">
            ...
            <div
              :class="[
                'w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border transition-all overflow-hidden bg-zinc-800 bg-opacity-40 border-zinc-700 border-opacity-30 group-hover:border-zinc-600 cursor-pointer hover:opacity-80',
                selectedSongs.includes(song.id) ? 'border-blue-500 border-opacity-50' : ''
              ]"
            ...
            <div class="min-w-0">
              ...
              >
                ...
                <button
                  ...
                  class="ml-2 inline-flex items-center justify-center w-5 h-5 rounded-full border border-blue-500 border-opacity-30 bg-blue-500 bg-opacity-10 text-blue-400 hover:bg-blue-500 bg-opacity-20 transition-all"
                ...
                <span
                  v-if="song.hasSubmissionNote && song.submissionNote"
                  class="ml-2 text-xs text-blue-400 text-opacity-80 truncate max-w-[200px] cursor-pointer hover:text-blue-400 transition-colors font-normal"
                  :title="locale.actions.viewRemark"
                  @click.stop="openSubmissionRemark(song)"
                >
                  {{
          ...
          >
              ...
              <div class="col-span-3 lg:col-span-1 text-center shrink-0">
                <button
                  ...
                  :class="[
                    ...
                    song.voteCount > 0
                      ? 'bg-pink-500 bg-opacity-10 text-pink-500 hover:bg-pink-500 bg-opacity-20 cursor-pointer'
                      : 'bg-zinc-950 bg-opacity-30 text-zinc-700 cursor-default'
                  ]"
            ...
            >
              <button
                class="p-2 bg-zinc-800 bg-opacity-50 text-blue-400 hover:bg-blue-600 hover:text-white rounded-xl transition-all border border-zinc-700 border-opacity-30"
              ...
              <button
                v-if="!song.played"
                class="p-2 bg-zinc-800 bg-opacity-50 text-emerald-400 hover:bg-emerald-600 hover:text-white rounded-xl transition-all border border-zinc-700 border-opacity-30"
              ...
              <button
                v-else
                class="p-2 bg-zinc-800 bg-opacity-50 text-zinc-400 hover:bg-zinc-600 hover:text-white rounded-xl transition-all border border-zinc-700 border-opacity-30"
              ...
              <button
                class="p-2 bg-zinc-800 bg-opacity-50 text-amber-500 hover:bg-amber-600 hover:text-white rounded-xl transition-all border border-zinc-700 border-opacity-30"
              ...
              <button
                class="p-2 bg-zinc-800 bg-opacity-50 text-red-400 hover:bg-red-600 hover:text-white rounded-xl transition-all border border-zinc-700 border-opacity-30"
    ...
    >
      <div
        v-if="selectedSongs.length > 0"
        class="sm:hidden fixed bottom-[70px] left-4 right-4 z-40 bg-zinc-900 bg-opacity-95 border border-zinc-800 rounded-2xl p-2 shadow-[0_8px_30px_rgb(0,0,0,0.5)] backdrop-blur-xl flex items-center justify-between"
      >
        ...
        <div class="flex items-center gap-2">
          <button
            class="flex flex-col items-center justify-center w-14 h-12 bg-zinc-800 hover:bg-zinc-700 text-emerald-400 rounded-xl border border-zinc-700 border-opacity-50 transition-all active:scale-95"
            @click="openDownloadDialog"
          >
          ...
          <button
            class="flex flex-col items-center justify-center w-14 h-12 bg-zinc-800 hover:bg-zinc-700 text-red-400 rounded-xl border border-zinc-700 border-opacity-50 transition-all active:scale-95"
            @click="batchDelete"
          >
        ...
        >
          ...
          <div class="p-8 space-y-6">
            <div class="p-5 bg-zinc-950 border border-zinc-800 rounded-lg flex items-center gap-4">
              <div
                class="w-10 h-10 rounded-lg bg-red-500 bg-opacity-10 text-red-500 flex items-center justify-center border border-red-500 border-opacity-10"
              >
                <Music :size="18" />
              </div>
              <div>
            ...
            <div class="space-y-2">
              ...
              <textarea
                ...
                class="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-4 text-sm text-zinc-200 focus:outline-none focus:border-red-500 border-opacity-30 min-h-[120px] resize-none transition-all"
              />
            </div>
          ...
          <div class="px-8 py-6 bg-zinc-900 bg-opacity-50 border-t border-zinc-800 border-opacity-50 flex gap-3 justify-end">
            <button
    ...
    >
      <div
        ...
        class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950 bg-opacity-80 backdrop-blur-sm overflow-y-auto"
      >
        <div
          ...
          <div class="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div class="space-y-2">
                ...
                <input
                  ...
                  class="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-3 text-sm text-zinc-200 focus:outline-none focus:border-blue-500 border-opacity-30 transition-all"
                />
                <input
                  v-else
            ...
            <div class="space-y-2">
              ...
              <div class="relative user-search-container">
                <div class="relative">
                  ...
                  >
                    <div
                      class="w-4 h-4 border-2 border-blue-500 border-opacity-20 border-t-blue-500 rounded-full animate-spin"
                    />
                  </div>
                </div>
            ...
            <div v-if="showEditModal" class="space-y-3">
                ...
                <button
                  ...
                  :class="
                    submissionNoteClearRequested
                      ...
                      : 'border-amber-500 border-opacity-20 bg-amber-500 bg-opacity-10 text-amber-400 hover:bg-amber-500 bg-opacity-20'
                  "
                  @click="
                    submissionNoteClearRequested
              ...
              <textarea
                ...
                class="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-4 text-sm text-zinc-200 focus:outline-none focus:border-blue-500 border-opacity-30 min-h-[120px] resize-none transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              />
              <label
              ...
              <div
                ...
                class="space-y-3 rounded-xl border border-amber-500 border-opacity-20 bg-amber-500 bg-opacity-5 p-4"
              >
                ...
                <textarea
                  ...
                  class="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-3 text-sm text-zinc-200 focus:outline-none focus:border-amber-500 border-opacity-30 min-h-[96px] resize-none transition-all"
                />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 pt-4 border-t border-zinc-800 border-opacity-50">
              <div class="space-y-2">
                <label class="text-[10px] font-black text-zinc-600 uppercase tracking-widest px-1"
            ...
            <div class="space-y-2">
              ...
              <p
                ...
                "
                class="text-[10px] text-emerald-500 text-opacity-80 font-bold px-1 flex items-center gap-1"
              >
              ...
              <p
                v-if="
                  ...
                  (showEditModal ? editForm.cover : addForm.cover)
                "
                class="text-[10px] text-red-500 text-opacity-80 font-bold px-1 flex items-center gap-1"
              >
                <X :size="10" />
