 ...
 * 处理单行歌词的词汇数据
 */
function processWords(line: ExistingLyricLine, lineEndTime: number, enableWordByWord: boolean): LyricWord[] {
  // 如果有逐字歌词数据，直接使用
 ...
 * 查找匹配的翻译歌词
 */
function findMatchingTranslation(line: ExistingLyricLine, translations: ExistingLyricLine[]): ExistingLyricLine | null {
...
export function smartWordSegmentation(
  ...
  const avgDuration = Math.max(minWordDuration, Math.min(maxWordDuration, totalDuration / segments.length))
  ...
  if (enableSmartSegmentation) {
    convertedLyrics = convertedLyrics.map(line => {
      if (line.words.length === 1 && line.words[0].word === line.words[0].word.trim()) {
        const smartWords = smartWordSegmentation(
          line.words[0].word,
          line.startTime,
          line.endTime
        )
