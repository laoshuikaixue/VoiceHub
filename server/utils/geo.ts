...
export const getServerLocation = async (): Promise<GeoLocation> => {
  ...
  try {
    ...
    // 尝试 ip-api.com
    const response: any = await $fetch('http://ip-api.com/json/?fields=status,message,country,countryCode,regionName,city', {
      timeout: 5000
    })
    ...
    console.warn('[Geo] Failed to detect server location, defaulting to non-China (conservative strategy):', error)
