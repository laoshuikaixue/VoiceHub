...
const githubStrategy: OAuthStrategy = {
    getAuthorizeUrl(redirectUri: string, state: string) {
        ...
        if (!clientId) throw createError({ statusCode: 500, message: 'GitHub Client ID not configured' })
...
const casdoorStrategy: OAuthStrategy = {
    getAuthorizeUrl(redirectUri: string, state: string) {
        ...
        if (!clientId || !endpoint) throw createError({ statusCode: 500, message: 'Casdoor config missing' })
