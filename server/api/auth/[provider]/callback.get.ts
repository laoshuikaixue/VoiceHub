...
async function handleUserLoginOrBind(event: any, provider: string, providerUserId: string, providerUsername: string) {
        // 4. 检查是否已登录（绑定模式）
        ...
        if (existingIdentity && existingIdentity.user) {
            ...
            return sendRedirect(event, `/login?action=bind&provider=${provider}&username=${encodeURIComponent(providerUsername)}`)
        }
}
