import {db, withStatementTimeout} from '~/drizzle/db'
...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        // 重新缓存完整的排期列表
        try {
            const completeSchedules = await withStatementTimeout(2000, async (tx: any) => {
                const schedulesData = await tx.select({
                    id: schedules.id,
                    playDate: schedules.playDate,
                    sequence: schedules.sequence,
                ...
                const voteCountResults = songIds.length > 0
                    ? await tx.select({songId: votes.songId, count: count()})
                        .from(votes)
                        .where(inArray(votes.songId, songIds))
                        .groupBy(votes.songId)
                    : []
                const voteCountMap = new Map(voteCountResults.map(v => [v.songId, v.count]))
