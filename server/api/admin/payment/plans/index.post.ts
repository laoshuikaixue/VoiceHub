...
export default defineEventHandler(async event => {
  ...
  if (!body?.name || !Number.isInteger(body.priceCents) || body.priceCents < 1 || (body.originalPriceCents !== null && body.originalPriceCents !== undefined && (!Number.isInteger(body.originalPriceCents) || body.originalPriceCents < 0)) || !Number.isInteger(body.cardCount) || body.cardCount < 1 || (!validityEmpty && (!Number.isInteger(validityValue) || validityValue < 1 || !['day', 'month', 'year'].includes(validityUnit))) || !/^[A-Z]{3}$/.test(currency)) throw createApiError(400, SERVER_ERROR_CODES.COMMON_INVALID_PARAMS, '套餐参数无效')
