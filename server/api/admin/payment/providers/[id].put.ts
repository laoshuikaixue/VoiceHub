...
export default defineEventHandler(async event => {
  ...
  const [updated] = await db.update(paymentProviderInstances).set({
    ...
    refundEnabled: typeof body.refundEnabled === 'boolean' ? body.refundEnabled : current.refundEnabled,
    allowUserRefund: typeof body.allowUserRefund === 'boolean' ? body.allowUserRefund : current.allowUserRefund,
