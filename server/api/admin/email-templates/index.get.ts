...
export default defineEventHandler(async (event) => {
  ...
  const merged = builtin.map(b => {
    ...
    return { ...b, isOverridden: !!override, overriddenAt: override?.updatedAt, overriddenByUserId: override?.updatedByUserId }
  })

  // 也返回额外的自定义模板（非内置键）
  const extra = custom.filter(c => !builtin.find(b => b.key === c.key)).map(c => ({
    key: c.key,
    name: c.name,
    subject: c.subject,
    html: c.html,
