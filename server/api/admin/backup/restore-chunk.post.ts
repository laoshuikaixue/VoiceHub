...
export default defineEventHandler(async (event) => {
    ...
    // 转换映射对象为Map
    const userIdMapping = new Map(Object.entries(mappings?.users || {}).map(([k, v]) => [Number(k), Number(v)]))
    const songIdMapping = new Map(Object.entries(mappings?.songs || {}).map(([k, v]) => [Number(k), Number(v)]))
                ...
                switch (tableName) {
                    case 'users': {
                        ...
                        if (mode === 'merge') {
                            ...
                            if (existingUser) {
                                createdUser = (await tx.update(users)
                    ...
                    case 'songs': {
                        ...
                        if (mode === 'merge') {
                            ...
                            if (existingSong) {
                                createdSong = (await tx.update(songs)
                                    .set(songData)
                                ...
                                try {
                                    ...
                                    if (existingSongWithId) {
                                        ...
                                        createdSong = (await tx.insert(songs).values({
                                            ...songData,
                                            id: record.id
                                        }).returning())[0]
                                    }
                    ...
                    case 'semesters': {
                        ...
                        if (mode === 'merge') {
                            ...
                            if (existingSemester) {
                                await tx.update(semesters).set(semesterData).where(eq(semesters.id, existingSemester.id))
                                stats.updated++
                            } else {
                            ...
                            const existing = await tx.query.semesters.findFirst({where: eq(semesters.id, record.id)})
                            if (existing) {
                    ...
                    case 'systemSettings': {
                        ...
                        if (mode === 'merge') {
                            ...
                            if (existing) {
                                await tx.update(systemSettings).set(systemSettingsData).where(eq(systemSettings.id, existing.id))
                                stats.updated++
                            } else {
                            ...
                            const existing = await tx.query.systemSettings.findFirst({where: eq(systemSettings.id, record.id)})
                            if (existing) {
                                await tx.update(systemSettings).set(systemSettingsData).where(eq(systemSettings.id, record.id))
                                stats.updated++
                            } else {
                    ...
                    case 'schedules': {
                        ...
                        if (mappedSongId) {
                            ...
                            const songExists = await tx.query.songs.findFirst({where: eq(songs.id, record.songId)})
                            if (!songExists) return
                        }

                        let validPlayTimeId = record.playTimeId
                        if (record.playTimeId) {
                            const ptExists = await tx.query.playTimes.findFirst({where: eq(playTimes.id, record.playTimeId)})
                        ...
                        if (mode === 'merge') {
                            const existing = await tx.query.schedules.findFirst({
                                where: and(eq(schedules.songId, validSongId), eq(schedules.playDate, scheduleData.playDate))
                            })
                            if (existing) {
                            ...
                            const existing = await tx.query.schedules.findFirst({where: eq(schedules.id, record.id)})
                            if (existing) {
                    ...
                    case 'notificationSettings': {
                        ...
                        if (record.userId) {
                            ...
                            if (mappedUserId) validUserId = mappedUserId
                                ...
                                const userExists = await tx.query.users.findFirst({where: eq(users.id, record.userId)})
                        ...
                        if (!notificationSettingsData.refreshInterval) notificationSettingsData.refreshInterval = 60
                        if (!notificationSettingsData.songVotedThreshold) notificationSettingsData.songVotedThreshold = 1
                        ...
                        if (mode === 'merge') {
                            const existing = await tx.query.notificationSettings.findFirst({where: eq(notificationSettings.userId, validUserId)})
                            if (existing) {
                                await tx.update(notificationSettings).set(notificationSettingsData).where(eq(notificationSettings.id, existing.id))
                                stats.updated++
                            } else {
                            ...
                            const existing = await tx.query.notificationSettings.findFirst({where: eq(notificationSettings.id, record.id)})
                            if (existing) {
                                await tx.update(notificationSettings).set(notificationSettingsData).where(eq(notificationSettings.id, record.id))
                                stats.updated++
                            } else {
                                await tx.insert(notificationSettings).values({...notificationSettingsData, id: record.id})
                                stats.created++
                            }
                        }
                        break
                    ...
                    case 'notifications': {
                        ...
                        if (mode === 'merge') {
                            ...
                            if (existing) {
                                await tx.update(notifications).set(notificationData).where(eq(notifications.id, existing.id))
                                stats.updated++
                            } else {
                            ...
                            const existing = await tx.query.notifications.findFirst({where: eq(notifications.id, record.id)})
                            if (existing) {
                                await tx.update(notifications).set(notificationData).where(eq(notifications.id, record.id))
                                stats.updated++
                            } else {
                    ...
                    case 'songBlacklist': {
                        ...
                        if (record.createdBy) {
                            ...
                            if (mappedUserId) validCreatedBy = mappedUserId
                                ...
                                const userExists = await tx.query.users.findFirst({where: eq(users.id, record.createdBy)})
                        ...
                        if (mode === 'merge') {
                            const existing = await tx.query.songBlacklists.findFirst({
                                where: and(eq(songBlacklists.type, blacklistData.type), eq(songBlacklists.value, blacklistData.value))
                            })
                            if (existing) {
                                await tx.update(songBlacklists).set(blacklistData).where(eq(songBlacklists.id, existing.id))
                                stats.updated++
                            } else {
                            ...
                            const existing = await tx.query.songBlacklists.findFirst({where: eq(songBlacklists.id, record.id)})
                            if (existing) {
                                await tx.update(songBlacklists).set(blacklistData).where(eq(songBlacklists.id, record.id))
                                stats.updated++
                            } else {
