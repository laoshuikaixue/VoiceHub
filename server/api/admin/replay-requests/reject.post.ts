...
export default defineEventHandler(async (event) => {
    ...
    // 获取歌曲信息用于通知
    const song = await db.select().from(songs).where(eq(songs.id, songId)).limit(1).then(r => r[0])
    if (!song) {
         throw createError({ statusCode: 404, message: '歌曲不存在' })
    }

    // 查找所有待处理的申请
    const pendingRequests = await db.select().from(songReplayRequests).where(and(
    ...
    for (const req of pendingRequests) {
        await createReplayRequestRejectedNotification(req.userId, {
            title: song.title,
            artist: song.artist
        }, ipAddress)
    }
