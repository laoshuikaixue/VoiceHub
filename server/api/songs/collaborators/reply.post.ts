...
export default defineEventHandler(async (event) => {
    ...
    try {
        // 查找联合投稿记录
        const collabRecord = await db.select().from(songCollaborators)
            .where(and(
        ...
        await db.update(songCollaborators)
            .set({
                status,
                updatedAt: new Date()
            })
        ...
        // 获取歌曲信息以发送通知
        const song = await db.select().from(songs).where(eq(songs.id, songId)).limit(1).then(res => res[0])
