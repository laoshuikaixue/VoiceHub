...
export default defineEventHandler(async (event) => {
  ...
  try {
    if (body.continuation) {
      ...
      return await resolvePlugins(next.track, next.quality, next.attempted, user.id, controller.signal)
    }
    const track = await requestTrack(body, user.id)
    return await resolvePlugins(track, ['standard', 'high', 'super', 'lossless'].includes(body.quality) ? body.quality : 'standard', Array.isArray(body.excludeSources) ? body.excludeSources.filter((v: unknown) => typeof v === 'string').slice(0, 40) : [], user.id, controller.signal)
